from image_center import get_foreground_bounds
...
def crop_square(filename, min_buffer):
    ...
    def crop_action(args):
        if type(args[3]) == bool:
            filename = args[0]
            min_buffer = int(args[1])
            return crop_square(filename, min_buffer)
        else:
            filename = args[0]
            width_buffer = int(args[1])
            height_buffer = int(args[2])
